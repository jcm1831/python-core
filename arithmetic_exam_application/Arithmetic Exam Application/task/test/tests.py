...
operations = ["+", "-", "*"]
wrong_format_msg = (
    'The task should have the following format: "number operation number"\n'
    '(e.g. "4 + 2", without quotes).\n'
    "However, the following output was found: "
)
corr_as_incorr_msg = (
    "The correct answer was given to the equation.\n"
    "The app should print 'Right!'. \n"
    "However, this word was not found in the output."
)
incorr_as_corr_msg = (
    "An incorrect answer was given to the equation.\n"
...
wrong_n_lines = (
    "The number of lines in the output is less than expected.\n"
...
def calc(task: str) -> Optional[int]:
    ...
    try:
        ...
        raise WrongAnswer(
            f"The format of the task {original_task} does not follow the required format.\n"
...
class ExamTest(StageTest):
    ...
    def test_level_1(self):
        ...
        if "3/5" in result:
            ...
            return CheckResult.wrong(
                "The number of correct answers is calculated incorrectly: \n"
                + final_output
                + "\nShould be '3/5'."
            )
        else:
            return CheckResult.wrong(
                "The final output doesn't contain count of correct answers: "
                + final_output
            )

        if "save" not in result:
            return CheckResult.wrong(
                "Your app should ask if the user want to save results in file."
            )
        ...
        if "saved" not in result.lower():
            return CheckResult.wrong(
                "The app should inform the user that the results were saved in the file."
            )
        ...
        if text_from_file not in text_from_file_after_saving:
            return CheckResult.wrong(
                "Your application shouldn't remove any information from the file with results "
                "if it already exists."
            )

        if all(
            [i in text_from_file_after_saving for i in ["Tester", "3/5", "1", "2", "9"]]
        ):
            return CheckResult.correct()
        else:
            return CheckResult.wrong(
                "The application didn't save the result in the needed format.\n"
    ...
    def test_level_2_numbers(self):
        ...
        for i in range(200):
            ...
            try:
                ...
                return CheckResult.wrong(
                    "Please, make sure that in level 2 your task is about integral squares:\n"
    ...
    def test_level_2_correct(self):
        ...
        try:
            ...
            raise WrongAnswer(
                "The task for level 2 should contain one number that the user should square.\n"
        ...
        if all(
            [
                i in text_from_file_after_saving
                for i in ["Tester", "2/5", "2", "11", "29"]
            ]
        ):
            return CheckResult.correct()
        else:
    ...
    def test_no_file(self):
        ...
        if not os.path.exists("results.txt"):
            return CheckResult.wrong(
                "Your app doesn't create the file 'results.txt' when it doesn't exist."
            )
